...
export const projectValidator = [
    ...
    body('firstName', 'Enter your real name').isLength({ min: 3 }),
    body('lastName', 'Enter your real last name').isLength({ min: 3 }),
    body('email', 'invalid mail format').isEmail(),
    body('phoneNumber', 'invalid phone number format').isLength({ min: 9, max: 13 }),
    body('avatarUrl', 'wrong image format').isString(),
